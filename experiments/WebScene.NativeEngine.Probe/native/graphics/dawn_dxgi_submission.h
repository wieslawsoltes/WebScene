#pragma once
#include "dawn_dxgi_canvas_texture.h"
...
class dawn_dxgi_submission final : public std::enable_shared_from_this<dawn_dxgi_submission> {
public:
...
public:
    ...
    class snapshot final {
        std::shared_ptr<dawn_dxgi_submission> submission_;
        std::optional<owned_image_pool::retained> image_;
        friend class dawn_dxgi_submission;
        snapshot(std::shared_ptr<dawn_dxgi_submission> submission,
    ...
    static std::shared_ptr<dawn_dxgi_submission> publish_submitted(
        d3d12_canvas_images::frame&& frame,const wgpu::Device& device,
        std::shared_ptr<dawn_dxgi_image> shared,std::shared_ptr<void> device_lifetime={},
        ...
        auto result=std::make_shared<dawn_dxgi_submission>();
        auto pending=std::make_shared<d3d12_canvas_images::frame>(std::move(frame));
        ...
        const bool ended=shared->end(end)==dxgi_access_status::success;
