...
#include "dawn_dxgi_canvas_host.h"
#if defined(_WIN32)
...
inline webgpu_canvas_host make_dxgi_webgpu_canvas_host(
    std::shared_ptr<dawn_dxgi_canvas_host> provider,
    ...
    result.validate=[](const webgpu_canvas_configuration& config) {
        if(config.format!=wgpu::TextureFormat::BGRA8Unorm||config.color_space!="srgb"||config.tone_mapping!="standard")
            throw std::invalid_argument("DXGI presenter currently requires BGRA8 sRGB standard tone mapping");
        if(!config.device.HasFeature(wgpu::FeatureName::SharedTextureMemoryDXGISharedHandle)||
            !config.device.HasFeature(wgpu::FeatureName::SharedFenceDXGISharedHandle))
            throw std::invalid_argument("Canvas device lacks native DXGI sharing capabilities");
    };
