...
try {
  ...
  const outputDirectory = resolve(
    valueAfter('--output-dir') ?? 'obj/htmlml-interop-validation');
  ...
  const manifestPath = join(
    outputDirectory,
    `${artifactName}.htmlml-interop-api.json`);
  const policyPath = join(
    outputDirectory,
    `${artifactName}.htmlml-interop-policy.json`);
  ...
  if (!skipCompile) {
    ...
    const dotnetArguments = [
      ...
      `-p:HtmlMLInteropApiManifest=${manifestPath}`,
      `-p:HtmlMLInteropPolicy=${policyPath}`,
      `-p:HtmlMLInteropHarnessId=${harnessId}`
    ];
