...
static int prepare_css_module(const fs::path& input,const fs::path& output,
  ...
  for(const auto& diagnostic:sheet->diagnostics) {
    ...
    std::cerr << input.string() << ": " << diagnostic.classification << ": "
        << diagnostic.feature << ": " << diagnostic.detail << '\n';
  }
  out << "return sheet;\n}\n}\n";
...
int main(int argc, char **argv) {
  ...
  if (!prepare_css && argc != 3 && argc != 5 && argc != 7) {
    std::cerr
