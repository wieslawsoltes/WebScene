...
static std::string compiled_mix_code(const std::string &value) {
    ...
    for (size_t i = 0; i < 2; ++i) {
      ...
      colors[i] = stop[0];
      if (ascii_keyword(colors[i].substr(0, 4)) != "var(" && !compiled_color(colors[i])) throw unsupported();
      if (stop.size() == 2) {
...
static std::string assignments(const std::string &name,
  ...
  if (ascii_keyword(value.substr(0, 10)) == "color-mix(") {
    std::string code = compiled_mix_code(value);
  ...
  if (name == "box-shadow") {
    ...
    if (ascii_keyword(value).find("var(") == std::string::npos) {
      ...
      for (const auto &part : components) {
        ...
        } else if (keyword == "currentcolor" || compiled_color(part) || keyword.starts_with("color-mix(")) {
    ...
    std::string code = "webscene::native_web::variable_result shadow=webscene::native_web::variable_tokens{};";
    for (const auto &part : components) {
      if (ascii_keyword(part).starts_with("color-mix(")) {
        code += "{" + compiled_mix_code(part) +
            "if(!mixed)shadow.reset();else if(shadow)shadow->emplace_back(\"\",std::nullopt,mixed);}";
      } else {
        code += "{auto part=s.evaluate(" + variable_code(part) +
            ");if(!part)shadow.reset();else if(shadow)shadow->insert(shadow->end(),part->begin(),part->end());}";
      }
    }
    return code + "s.set_box_shadow(shadow);";
  }
...
static void emit_prepared_sheet(std::ostream&,const css::prepared_stylesheet&);
struct compiler {
  bool preview{};
  bool shared_css{};
  std::vector<css::prepared_stylesheet> prepared_sheets;
  ...
  void stylesheet(const std::string &text, bool exact_locations = false,
                  ...
                  std::optional<bool> initial_motion = {}, std::string media = {}) {
    if(shared_css) {
      if(!media.empty()) media="@media " + media + "{" + text + "}";
      auto sheet=css::prepare_stylesheet(media.empty()?text:media,source.string(),[](const auto& query) {
        return css::inventory_media(query,[](std::string_view,std::string_view,std::string_view,const std::string&,std::string_view) {});
      });
      if(!sheet) throw std::runtime_error("shared CSS preparation failed");
      for(const auto& diagnostic:sheet->diagnostics)
        std::cerr << source.string() << ": " << diagnostic.classification << ": " << diagnostic.feature << ": " << diagnostic.detail << '\n';
      prepared_sheets.push_back(std::move(*sheet));
      return;
    }
  ...
  void node(const dom_node &n, const std::string &parent) {
    ...
    static const std::set<std::string> tags = {
        // Native DOM text/structure, label activation, dialogs and image nodes
        // are also available to compiled views (including reusable templates).
        "strong", "b", "em", "i", "small", "code", "kbd", "samp", "var",
        "abbr", "cite", "q", "s", "u", "sub", "sup", "mark", "time",
        "h4", "h5", "h6", "address", "blockquote", "pre", "figure", "figcaption",
        "ol", "dl", "dt", "dd", "label", "dialog", "img",
    ...
    for (const auto &[k, v] : attrs) {
      if (k == "style" && shared_css) {
        out << "d.attribute(" << local << ",\"style\"," << quote(v) << ");\n";
        continue;
      }
      if (k == "style") {
      ...
      if (k != "title" && k != "hidden" && k != "id" && k != "class" && k != "width" && k != "height" &&
          ...
          !((n.tag == "input" || n.tag == "textarea") &&
            (k == "value" || k == "placeholder" || k == "readonly" || k == "checked" || k == "autocomplete")) &&
          !(n.tag == "label" && k == "for") &&
          !(n.tag == "dialog" && k == "open") &&
          !(n.tag == "img" && (k == "src" || k == "alt")) &&
  ...
  void compile(const fs::path &input, const fs::path &output,
    ...
    const auto api_header=shared_css?"webscene/shared_css.hpp":"webscene/native_web.hpp";
    ...
    const auto walk = [&](auto &&self, const dom_node &n) -> void {
      ...
      if (!shared_css && (n.tag == "style" || n.tag == "link") && n.attributes.contains("media")) {
      ...
      if (n.tag == "style") {
        ...
        stylesheet(css, at != std::string::npos, media_bounds, media_motion, n.attributes.contains("media")?n.attributes.at("media"):"");
      ...
      if (n.tag == "link") {
        ...
        content = read(p);
        source = p;
        stylesheet(content, true, media_bounds, media_motion, n.attributes.contains("media")?n.attributes.at("media"):"");
        content = saved;
        source = saved_path;
      }
      for (auto *c : n.children)
    ...
    for (const auto &[key, value] : body->parent->attributes) {
      ...
      if (key == "style" && shared_css) {
        out << "d.attribute(d.root(),\"style\"," << quote(value) << ");\n";
        continue;
      }
      if (key == "style") {
    ...
    if(shared_css) prefix << ",std::shared_ptr<webscene::native_web::shared_css_report> report={}";
    prefix << ") {\n";
    if(shared_css) {
      prefix << "std::vector<webscene_native::css::prepared_stylesheet> sheets;\n";
      for(const auto& sheet:prepared_sheets) {
        prefix << "sheets.push_back([](){\n";
        emit_prepared_sheet(prefix,sheet);
        prefix << "}());\n";
      }
      prefix << "d.set_stylesheet_resolver(webscene::native_web::make_shared_stylesheet_resolver(std::move(sheets),std::move(report)));\n";
    }
    prefix << out.str() << "return {";
    for (size_t i = 0; i < names.size(); ++i) {
      if (i)
        prefix << ",";
...
// Emit parser output, not a second implementation of CSS syntax. Textual values
// remain in this transitional representation and are interpreted by shared CSS.
static void emit_prepared_selector(std::ostream& out, std::string_view target,
                                  const css::compiled_css_selector& selector) {
  out << target << ".specificity=" << selector.specificity << "u;\n";
  for(const auto& part:selector.compounds)
    out << target << ".compounds.push_back(" << quote(part) << ");\n";
  for(unsigned char relation:selector.combinators)
    out << target << ".combinators.push_back(static_cast<char>(" << unsigned(relation) << "));\n";
...
static void emit_prepared_sheet(std::ostream& out,const css::prepared_stylesheet& sheet) {
  ...
  // Stable output across unordered-map iteration and separate compiler runs.
  std::map<std::string,css::css_opacity_keyframes> keyframes(sheet.keyframes.begin(),sheet.keyframes.end());
  for(const auto& [name,frames]:keyframes) {
    out << "{auto& k=sheet.keyframes[" << quote(name) << "];\n";
    for(const auto& stop:frames.opacity_stops)
      out << "k.opacity_stops.push_back({" << number(stop.offset) << ',' << number(stop.opacity) << "});\n";
    for(const auto& stop:frames.rotation_stops)
      out << "k.rotation_stops.push_back({" << number(stop.offset) << ',' << number(stop.degrees) << "});\n";
    out << "}\n";
  }
  for(const auto& diagnostic:sheet.diagnostics) {
    out << "sheet.diagnostics.push_back({" << quote(diagnostic.feature) << ','
        << quote(diagnostic.classification) << ',' << quote(diagnostic.detail) << "});\n";
  }
  out << "return sheet;\n";
}
static int prepare_css_module(const fs::path& input,const fs::path& output,
  ...
  if(fs::weakly_canonical(input)==fs::weakly_canonical(output))
    throw std::runtime_error("CSS output must not overwrite its input");
  auto sheet=css::prepare_stylesheet(read(input),fs::absolute(input).string(),[](const auto& query) {
    return css::inventory_media(query,[](std::string_view,std::string_view,std::string_view,const std::string&,std::string_view) {});
  });
  if(!sheet) throw std::runtime_error("CSS preparation failed");
  std::ostringstream out;
  out << "// Generated by webscene-uic --prepare-css. Text values still require shared CSS evaluation.\n"
      << "module;\n#include <webscene_css_stylesheet_data.h>\n"
      << "export module " << module << ";\nexport namespace compiled_css {\n"
      << "inline webscene_native::css::prepared_stylesheet build() {\n"
      ;
  emit_prepared_sheet(out,*sheet);
  out << "}\n}\n";
  for(const auto& diagnostic:sheet->diagnostics)
    std::cerr << input.string() << ": " << diagnostic.classification << ": " << diagnostic.feature << ": " << diagnostic.detail << '\n';
  std::ofstream file(output,std::ios::binary);
  if(!file) throw std::runtime_error("cannot write prepared CSS module");
...
int main(int argc, char **argv) {
  ...
  c.preview = preview;
  try {
    if(prepare_css) {c.source=argv[2];return prepare_css_module(argv[2],argv[3],argv[5]);}
    ...
    for (int option = 3; option < argc; option += 2) {
      if (std::string_view(argv[option]) == "--css-backend") {
        const std::string_view backend=argv[option+1];
        if(backend!="shared" && backend!="typed") throw std::runtime_error("Unknown CSS backend");
        c.shared_css=backend=="shared";
        continue;
      }
