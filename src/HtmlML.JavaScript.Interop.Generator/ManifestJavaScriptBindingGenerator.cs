...
public sealed class ManifestJavaScriptBindingGenerator : IIncrementalGenerator
{
        ...
        string policyPath)
    {
        ...
        foreach (var model in models)
        {
            ...
            context.AddSource(
                $"{Sanitize(namespaceName)}_{Sanitize(model.Name)}.HtmlMLModel.g.cs",
        ...
        foreach (var adapter in adapters)
        {
            ...
            context.AddSource(
                $"{Sanitize(namespaceName)}_{Sanitize(adapter.Name)}.HtmlMLAdapter.g.cs",
                SourceText.From(source, Encoding.UTF8));
        }
        foreach (var binding in bindings)
        {
            ...
            context.AddSource(
                $"{Sanitize(namespaceName)}_{Sanitize(binding.Name)}.HtmlMLManifestInterop.g.cs",
                SourceText.From(source, Encoding.UTF8));
        }
        if (functions.Length > 0 || globalProperties.Length > 0)
        {
            ...
            var source = GenerateGlobalFunctions(
                ...
                globalProperties);
            context.AddSource(
                $"{Sanitize(namespaceName)}_{Sanitize(functionsClassName)}.HtmlMLGlobalFunctions.g.cs",
                SourceText.From(source, Encoding.UTF8));
        }
        for (var index = 0; index < generation.StructuralModels.Count; index++)
        {
            ...
            context.AddSource(
                $"{Sanitize(namespaceName)}_{Sanitize(structural.Name)}.HtmlMLStructuralModel.g.cs",
        ...
        GlobalProperty property)
    {
        ...
        source.AppendLine()
            ...
            .AppendLine("        global::HtmlML.JavaScript.Interop.IJavaScriptInvoker invoker,")
        ...
        if (mapping.IsBinding && mapping.IsNullable)
        {
            source.Append("        var reference = await invoker.")
                ...
                .Append("<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference?>(")
        ...
        else if (mapping.IsFunctionReference && mapping.IsNullable)
        {
            source.Append("        var reference = await invoker.")
                ...
                .AppendLine("            ? new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(invoker, value)")
                .AppendLine("            : null;");
        }
        else if (mapping.IsFunctionReference)
        {
            source.Append("        return new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(invoker, await invoker.")
        ...
        GlobalFunction function)
    {
        ...
        var parameters = method.GetProperty("parameters")
            ...
            .Select(parameter =>
            {
                ...
                if (optional)
                {
                    ...
                    {
                        CSharpType =
                            $"global::HtmlML.JavaScript.Interop.JavaScriptOptional<{mapping.CSharpType}>"
                    };
                }
        ...
        if (promise)
        {
            ...
            {
                source.Append("        var reference = await invoker.InvokeGlobalPromiseAsync<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference?>(")
                    .Append(Literal(function.GlobalName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false);")
                    .Append("        return reference is { } value ? new ")
            ...
            {
                source.Append("        return new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(invoker, await invoker.InvokeGlobalPromiseObjectAsync(")
                    .Append(Literal(function.GlobalName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false));");
            }
        ...
        {
            source.Append("        var reference = await invoker.InvokeGlobalAsync<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference?>(")
                .Append(Literal(function.GlobalName)).Append(", ")
                .Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false);")
                .Append("        return reference is { } value ? new ")
        ...
        {
            source.Append("        return new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(invoker, await invoker.InvokeGlobalObjectAsync(")
                .Append(Literal(function.GlobalName)).Append(", ")
                .Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false));");
        }
        ...
        string policyPath)
    {
        ...
        source.Append("namespace ").Append(generation.Namespace).AppendLine(";")
            ...
            .AppendLine(" : global::HtmlML.JavaScript.Interop.IJavaScriptCallbackTarget, global::HtmlML.JavaScript.Interop.IJavaScriptObjectReferenceProvider, global::System.IAsyncDisposable")
            .AppendLine("{")
            .AppendLine("    private global::HtmlML.JavaScript.Interop.IJavaScriptBidirectionalInvoker? __htmlMlInvoker;")
            .AppendLine("    private global::HtmlML.JavaScript.Interop.JavaScriptObjectReference __htmlMlReference;")
            .AppendLine()
            .AppendLine("    public global::HtmlML.JavaScript.Interop.JavaScriptObjectReference JavaScriptReference => __htmlMlReference;")
            .AppendLine()
            .AppendLine("    public async global::System.Threading.Tasks.ValueTask<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference> RegisterAsync(")
            .AppendLine("        global::HtmlML.JavaScript.Interop.IJavaScriptBidirectionalInvoker invoker,")
            .AppendLine("        global::System.Threading.CancellationToken cancellationToken = default)")
            .AppendLine("    {")
            .AppendLine("        global::System.ArgumentNullException.ThrowIfNull(invoker);")
            .AppendLine("        if (!__htmlMlReference.IsEmpty)")
            .AppendLine("        {")
            ...
            .AppendLine("        __htmlMlReference = await invoker.RegisterCallbackTargetAsync(")
            .AppendLine("            this,")
            .AppendLine("            new global::HtmlML.JavaScript.Interop.JavaScriptCallbackMethod[]")
            .AppendLine("            {");
        foreach (var method in selectedMethods)
        {
            source.Append("                new(").Append(Literal(method.SourceName))
                .Append(", global::HtmlML.JavaScript.Interop.JavaScriptCallbackReturnKind.")
            ...
            {
                source.Append(", global::HtmlML.JavaScript.Interop.JavaScriptJson.Serialize(")
        ...
        source.AppendLine("            },")
            .AppendLine("            cancellationToken).ConfigureAwait(false);")
            .AppendLine("        return __htmlMlReference;")
            .AppendLine("    }")
            .AppendLine()
            .AppendLine("    public async global::System.Threading.Tasks.ValueTask DisposeAsync()")
            .AppendLine("    {")
            .AppendLine("        if (__htmlMlInvoker is not null && !__htmlMlReference.IsEmpty)")
            .AppendLine("        {")
            .AppendLine("            await __htmlMlInvoker.ReleaseAsync(__htmlMlReference).ConfigureAwait(false);")
            .AppendLine("            __htmlMlReference = default;")
            .AppendLine("        }")
            .AppendLine("    }");
    ...
    {
        source.AppendLine()
            .AppendLine("    async global::System.Threading.Tasks.ValueTask<object?> global::HtmlML.JavaScript.Interop.IJavaScriptCallbackTarget.DispatchAsync(")
            ...
            .AppendLine("    {")
            .AppendLine("        var invoker = __htmlMlInvoker ?? throw new global::System.InvalidOperationException(\"The callback adapter is not registered.\");")
        ...
        foreach (var method in methods)
        {
            ...
            for (var index = 0; index < parameters.Length; index++)
            {
                ...
                {
                    source.Append("                    global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.HasValue(arguments, ")
                        .Append(index).AppendLine(")")
                        .Append("                        ? new global::HtmlML.JavaScript.Interop.JavaScriptOptional<")
                        .Append(mapping.CSharpType).AppendLine(">(")
                        .Append("                            global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.IsNull(arguments, ")
                        .Append(index).AppendLine(")")
                        .AppendLine("                                ? null")
                        .Append("                                : new ")
                        .Append(mapping.NonNullableCSharpType)
                        .Append("(invoker, global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.Get<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference>(arguments, ")
                ...
                else if (optional && mapping.IsCallbackWrapper)
                {
                    source.Append("                    global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.HasValue(arguments, ")
                        ...
                        .Append("                                : new ")
                        .Append(mapping.NonNullableCSharpType)
                        .Append("(invoker, global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.GetFunctionReference(arguments, ")
                        .Append(index).AppendLine(", invoker)))")
                        .AppendLine("                        : default,");
                }
                else if (optional)
                {
                    source.Append("                    global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.GetOptional<")
                ...
                {
                    source.Append("                    global::HtmlML.JavaScript.Interop.JavaScriptCallbackArguments.Get<")
                        .Append(mapping.CSharpType).Append(">(arguments, ").Append(index)
                        .AppendLine(", invoker)!,");
                }
            }
        ...
        ISet<string>? erasedTypeParameters = null)
    {
            ...
            && ContainsTypeParameter(type, erasedTypeParameters))
        {
            ...
            var erased = callback
                ? "global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference"
        ...
        string member)
    {
        ...
        {
            return "global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference";
        }
        ...
        {
            return "global::HtmlML.JavaScript.Interop.JavaScriptAction";
        }
        ...
        return "global::HtmlML.JavaScript.Interop.JavaScriptTupleAction<"
        ...
        IReadOnlyList<string> typeParameters)
    {
        ...
        source.Append('(').Append(wireDeclarationName)
            .Append(" wire, global::HtmlML.JavaScript.Interop.IJavaScriptInvoker invoker");
        ...
        {
            var wireValue =
                "global::HtmlML.JavaScript.Interop.JavaScriptInteropSerializer.Deserialize<"
        ...
        string policyPath)
    {
        ...
        source.Append("namespace ").Append(generation.Namespace).AppendLine(";")
            ...
            .AppendLine(" : global::HtmlML.JavaScript.Interop.IJavaScriptObjectReferenceProvider, global::System.IAsyncDisposable")
            .AppendLine("{")
        ...
        source.AppendLine()
            ...
            .AppendLine("        global::HtmlML.JavaScript.Interop.IJavaScriptInvoker invoker,")
            .AppendLine("        global::HtmlML.JavaScript.Interop.JavaScriptObjectReference reference)")
        ...
        string policyPath)
    {
        ...
        if (mapping.IsBinding && mapping.IsNullable)
        {
            source.Append("        var reference = await __htmlMlInvoker.")
                .Append(valueInvocation)
                .Append("<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference?>(")
                .Append("__htmlMlReference, ").Append(Literal(memberName))
                .AppendLine(", cancellationToken).ConfigureAwait(false);")
                .Append("        return reference is { } value ? new ")
                .Append(mapping.NonNullableCSharpType)
                .AppendLine("(__htmlMlInvoker, value) : null;");
        }
        else if (mapping.IsBinding)
        {
            source.Append("        return new ").Append(mapping.CSharpType)
                .Append("(__htmlMlInvoker, await __htmlMlInvoker.")
                .Append(objectInvocation).Append('(')
                .Append("__htmlMlReference, ").Append(Literal(memberName))
                .AppendLine(", cancellationToken).ConfigureAwait(false));");
        }
        else if (mapping.IsFunctionReference && mapping.IsNullable)
        {
            source.Append("        var reference = await __htmlMlInvoker.")
                ...
                .AppendLine(", cancellationToken).ConfigureAwait(false);")
                .AppendLine("        return reference is { } value")
                .AppendLine("            ? new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(__htmlMlInvoker, value)")
                .AppendLine("            : null;");
        }
        else if (mapping.IsFunctionReference)
        {
            source.Append("        return new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(__htmlMlInvoker, await __htmlMlInvoker.")
                .Append(objectInvocation).Append('(')
                .Append("__htmlMlReference, ").Append(Literal(memberName))
                .AppendLine(", cancellationToken).ConfigureAwait(false));");
        }
        ...
        {
            source.Append("        return await __htmlMlInvoker.")
                .Append(objectInvocation).Append('(')
                .Append("__htmlMlReference, ").Append(Literal(memberName))
                .AppendLine(", cancellationToken).ConfigureAwait(false);");
        }
        ...
        {
            source.Append("        var wire = (await __htmlMlInvoker.")
                .Append(valueInvocation).Append('<')
                .Append(mapping.EffectiveWireCSharpType)
                .Append(">(__htmlMlReference, ").Append(Literal(memberName))
                .AppendLine(", cancellationToken).ConfigureAwait(false))!;")
                .Append("        return ")
                .Append(mapping.ConvertFromWire("wire", "__htmlMlInvoker"))
                .AppendLine(";");
        }
        else
        {
            source.Append("        return (await __htmlMlInvoker.")
                .Append(valueInvocation).Append('<')
                .Append(mapping.CSharpType).Append(">(__htmlMlReference, ")
        ...
        var setterMapping = promise
            ? new TypeMapping(
                "global::HtmlML.JavaScript.Interop.JavaScriptObjectReference",
                IsBinding: false,
        ...
        source.AppendLine()
            ...
            .AppendLine("        global::System.Threading.CancellationToken cancellationToken = default)")
            .AppendLine("        => __htmlMlInvoker.SetPropertyAsync(")
            .AppendLine("            __htmlMlReference,")
        ...
        if (setterMapping.IsBinding && setterMapping.IsNullable)
        {
            source.AppendLine("value is null")
                .AppendLine("                ? global::HtmlML.JavaScript.Interop.JavaScriptArgument.From<object?>(null)")
                .AppendLine("                : global::HtmlML.JavaScript.Interop.JavaScriptArgument.From(value.__HtmlMlReference),");
        }
        else
        {
            source.Append("global::HtmlML.JavaScript.Interop.JavaScriptArgument.From(")
                .Append(setterMapping.IsBinding
                    ? "value.__HtmlMlReference"
                    : "value")
                .AppendLine("),");
        }
        source
        ...
        string policyPath)
    {
        ...
        if (promise)
        {
            if (returnsVoid)
            {
                source.Append("        await __htmlMlInvoker.InvokePromiseAsync<global::System.Text.Json.JsonElement>(")
                    .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false);");
            }
            else if (returnMapping.IsBinding && returnMapping.IsNullable)
            {
                source.Append("        var reference = await __htmlMlInvoker.InvokePromiseAsync<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference?>(")
                    .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false);")
                    .Append("        return reference is { } value ? new ")
            ...
            {
                source.Append("        return new ").Append(returnMapping.CSharpType)
                    .Append("(__htmlMlInvoker, await __htmlMlInvoker.InvokePromiseObjectAsync(")
                    .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false));");
            }
            ...
            {
                source.Append("        return new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(__htmlMlInvoker, await __htmlMlInvoker.InvokePromiseObjectAsync(")
                    .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false));");
            }
            ...
            {
                source.Append("        return await __htmlMlInvoker.InvokePromiseObjectAsync(")
                    .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                    .Append(arguments)
                    .AppendLine(", cancellationToken).ConfigureAwait(false);");
            }
            ...
            {
                source.Append("        var wire = (await __htmlMlInvoker.InvokePromiseAsync<")
            ...
            {
                source.Append("        return (await __htmlMlInvoker.InvokePromiseAsync<")
                    .Append(returnMapping.CSharpType).Append(">(__htmlMlReference, ")
        ...
        {
            source.Append("        await __htmlMlInvoker.InvokeVoidAsync(__htmlMlReference, ")
                .Append(Literal(memberName)).Append(", ").Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false);");
        }
        else if (returnMapping.IsBinding && returnMapping.IsNullable)
        {
            source.Append("        var reference = await __htmlMlInvoker.InvokeAsync<global::HtmlML.JavaScript.Interop.JavaScriptObjectReference?>(")
                .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                .Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false);")
                .Append("        return reference is { } value ? new ")
        ...
        {
            source.Append("        return new ").Append(returnMapping.CSharpType)
                .Append("(__htmlMlInvoker, await __htmlMlInvoker.InvokeObjectAsync(")
                .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                .Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false));");
        }
        ...
        {
            source.Append("        return new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(__htmlMlInvoker, await __htmlMlInvoker.InvokeObjectAsync(")
                .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                .Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false));");
        }
        ...
        {
            source.Append("        return await __htmlMlInvoker.InvokeObjectAsync(")
                .Append("__htmlMlReference, ").Append(Literal(memberName)).Append(", ")
                .Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false);");
        }
        ...
        {
            source.Append("        var wire = (await __htmlMlInvoker.InvokeAsync<")
                .Append(returnMapping.EffectiveWireCSharpType)
                .Append(">(__htmlMlReference, ").Append(Literal(memberName))
                .Append(", ").Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false))!;")
        ...
        {
            source.Append("        return (await __htmlMlInvoker.InvokeAsync<")
                .Append(returnMapping.CSharpType).Append(">(__htmlMlReference, ")
                .Append(Literal(memberName)).Append(", ").Append(arguments)
                .AppendLine(", cancellationToken).ConfigureAwait(false))!;");
        }
        ...
        string member)
    {
        ...
        string? csharpType = kind switch
        {
            ...
            "callback" => "global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference",
            ...
            "null" or "undefined"
                => "global::HtmlML.JavaScript.Interop.JavaScriptNullish",
        ...
        else if (isFunctionReference)
        {
            wireCSharpType =
                ...
                + (csharpType.EndsWith("?", StringComparison.Ordinal) ? "?" : "");
            fromWireTemplate = csharpType.EndsWith("?", StringComparison.Ordinal)
                ? "__VALUE__ is { } reference ? new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(__INVOKER__, reference) : null"
                : "new global::HtmlML.JavaScript.Interop.JavaScriptFunctionReference(__INVOKER__, __VALUE__)";
        }
        return new TypeMapping(
            csharpType,
            isBinding,
    ...
    private static string JavaScriptUnionType(IReadOnlyList<string> types)
        => "global::HtmlML.JavaScript.Interop.JavaScriptUnion<"
           + (types.Count <= 16
               ? string.Join(", ", types)
               : TupleType(types))
           + ">";
    ...
    {
        var publicType =
            "global::HtmlML.JavaScript.Interop.JavaScriptOptional<"
            + value.CSharpType
            + ">";
        ...
        JsonElement type)
    {
        ...
        if (name is "HTMLElement" or "Element" or "EventTarget" or "Window"
            ...
            or "BigUint64Array")
        {
            return "global::HtmlML.JavaScript.Interop.JavaScriptObjectReference";
        }
        if (name is "Object")
        {
        ...
        bool ignoreUndefinedNullability = false)
    {
        ...
        {
            return "global::HtmlML.JavaScript.Interop.JavaScriptNullish";
        }
        if (candidates.Length == 1)
        {
            var mapped = MapType(
    ...
    private static string ArgumentArray(IEnumerable<Parameter> parameters)
        => "new global::HtmlML.JavaScript.Interop.JavaScriptArgument[] { "
           + string.Join(
               ", ",
               parameters.Select(parameter =>
                   parameter.Optional
                       ...
                       : parameter.Rest
                           ? "global::HtmlML.JavaScript.Interop.JavaScriptArgument.FromRest(@"
                             + parameter.Name + ")"
                       : "global::HtmlML.JavaScript.Interop.JavaScriptArgument.From(@"
                         + parameter.Name
