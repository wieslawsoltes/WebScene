...
public sealed partial class ManifestJavaScriptBindingGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor ExternalMappingWithoutBinaryCodec = new(
        "WEBSCENEJS004",
        "External JavaScript type mapping has no ABI 3 codec",
        "TypeScript type '{0}' maps to external CLR type '{1}' without a generated ABI 3 binary codec ({2}); NativeJavaScriptInvoker cannot transport this type. Remove the type mapping and enable the generated model, or supply a custom binary invocation.",
        "WebScene.Interop",
        DiagnosticSeverity.Warning,
        isEnabledByDefault: true);
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(
            apiFiles.Collect().Combine(policyFiles.Collect()).Combine(context.CompilationProvider),
            static (productionContext, inputs) =>
                Emit(productionContext, inputs.Left.Left, inputs.Left.Right, inputs.Right));
    }
    ...
    {
        var externalCodecNames = new HashSet<string>(StringComparer.Ordinal);
    ...
    private static void EmitPolicy(
        ...
        string policyPath,
        Compilation compilation,
        ISet<string> externalCodecNames)
    {
        ...
        foreach (var codec in generation.ExternalCodecs.Values)
        {
        ...
        ISet<string> ExternalCodecNames)
    {
        ...
        private int _nextExternalCodec;

        public Dictionary<string, ExternalModelCodec> ExternalCodecs { get; } = new(StringComparer.Ordinal);
        public Dictionary<string, string> ExternalCodecFailures { get; } = new(StringComparer.Ordinal);
        public HashSet<string> PreparingExternalCodecs { get; } = new(StringComparer.Ordinal);

        public HashSet<string> ReportedExternalMappings { get; } = new(StringComparer.Ordinal);
        ...
        public string NextExternalCodecName()
        {
            string name;
            do
            {
                name = "__WebSceneExternalCodec" + _nextExternalCodec++;
            }
            while (IsGeneratedNameUsed(name)
                || name == (OptionalString(Policy, "functionsClassName") ?? "JavaScriptGlobals")
                || !ExternalCodecNames.Add(Namespace + "." + name));
            return name;
        }
        ...
        private bool IsGeneratedNameUsed(string name)
            => BindingNames.Values.Contains(name, StringComparer.Ordinal)
               ...
               || ExternalCodecs.Values.Any(codec => codec.Name == name)
