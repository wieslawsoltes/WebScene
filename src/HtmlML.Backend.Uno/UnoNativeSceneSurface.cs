...
/// </summary>
public sealed unsafe class UnoNativeSceneSurface : SKCanvasElement, INativeHtmlMlRenderDiagnostics
{
    ...
    public void SetEngine(IntPtr engine)
    {
        ...
        NativeHtmlMlApi.EngineSetVisible(_engine, IsLoaded ? (byte)1 : (byte)0);
    ...
    public ulong SubmitResize(double width, double height)
    {
        ...
        LastResizeSequence = NativeHtmlMlApi.EngineEnqueue(_engine, in input) != 0
            ? sequence
            : 0;
    ...
    protected override void OnApplyTemplate()
    {
        ...
        if (!IsSupportedOnCurrentPlatform())
        {
            throw new PlatformNotSupportedException(
                "HtmlML's Uno scene backend requires Uno's Skia renderer.");
        }
    }
    ...
    private bool EnqueueInput(in InputEvent input)
    {
        ...
        if (_engine == IntPtr.Zero
            || NativeHtmlMlApi.EngineEnqueue(_engine, in input) == 0)
        {
            return false;
        }
...
public sealed class UnoNativeHtmlMlView : ContentControl, IAsyncDisposable
{
    private readonly UnoNativeSceneSurface _surface = new();
    private IntPtr _engine;

    public UnoNativeHtmlMlView()
    {
        ...
        uint timeoutMilliseconds = 5_000)
    {
        ...
        return _engine != IntPtr.Zero
            && NativeHtmlMlApi.TryEvaluateJson(
                _engine,
                source,
                documentName,
                out json,
    ...
    public bool TryTakeHostRequest(out string request)
    {
        ...
        return NativeHtmlMlApi.TryTakeHostRequest(_engine, out request);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await NativeHtmlMlRuntime.PrewarmAsync(nativeLibraryPath, cancellationToken);
        ...
        if (_engine == IntPtr.Zero)
        {
            throw new InvalidOperationException("The HtmlML native engine could not be created.");
        }

        _surface.SetEngine(_engine);
        if (!NativeHtmlMlApi.TryLoadUrl(_engine, source))
        {
            throw new InvalidOperationException(
                $"Native HtmlML rejected {source}: {NativeHtmlMlApi.GetLastError(_engine)}");
        }
        ...
        {
            return NativeHtmlMlApi.TryEvaluateJson(
                _engine,
                "({ hasDocumentElement: !!document.documentElement, hasBody: !!document.body })",
                "htmlml-uno-document-barrier.js",
                out var json,
        ...
        {
            throw new InvalidOperationException(
                $"Native HtmlML did not construct a document for {source}: " +
                NativeHtmlMlApi.GetLastError(_engine));
        }
    ...
    public ValueTask DisposeAsync()
    {
        ...
        if (_engine != IntPtr.Zero)
        {
            NativeHtmlMlApi.EngineDestroy(_engine);
            _engine = IntPtr.Zero;
        }
...
internal sealed class UnoHttpResourceLoader : IHtmlMlResourceLoader
{
    ...
    public HtmlMlTextResource LoadText(in HtmlMlResourceRequest request)
    {
        ...
        var resource = new HtmlMlTextResource(address, content, address, null)
        {
    ...
    private static string Resolve(string specifier, string? baseAddress)
    {
        ...
        throw new InvalidOperationException($"Unable to resolve HtmlML resource '{specifier}'.");
    }
}
