...
typedef HtmlMlHostRequestHandler = FutureOr<void> Function(
  HtmlMlController controller,
  Map<String, dynamic> request,
);

final class HtmlMlConsoleMessage {
  const HtmlMlConsoleMessage(this.level, this.message);

  final String level;
  final String message;
}

/// A Flutter surface backed by the native HtmlML V8/DOM/scene runtime.
class HtmlMlView extends StatefulWidget {
  const HtmlMlView({
  ...
  final HtmlMlRuntimeConfiguration runtime;
  final HtmlMlController? controller;
  final List<HtmlMlScript> initializationScripts;
  final HtmlMlHostRequestHandler? onHostRequest;
  final VoidCallback? onReady;
  final ValueChanged<HtmlMlConsoleMessage>? onConsoleMessage;
  final ValueChanged<Object>? onError;
  final ValueChanged<int>? onScenePresented;

  @override
  State<HtmlMlView> createState() => _HtmlMlViewState();
}

class _HtmlMlViewState extends State<HtmlMlView>
    with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  final _projector = HtmlMlSceneProjector();
  final _focusNode = FocusNode(debugLabel: 'HtmlML');
  late HtmlMlController _controller;
  ...
  void initState() {
    ...
    _controller = widget.controller ?? HtmlMlController();
    unawaited(_start());
  }

  @override
  void didUpdateWidget(covariant HtmlMlView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.controller != widget.controller) {
      final engine = _engine;
      final nextController = widget.controller ?? HtmlMlController();
      if (engine != null) {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      final engine = HtmlMlEngine.create(
  ...
  void _onFrame(Duration elapsed) {
    ...
    if (size != _lastSize || scale != _lastScale) {
      ...
      engine.enqueue(kind: htmlMlFrame, x: timestamp);
    }
    ...
    if (elapsed.inMilliseconds - _lastConsoleDrain >= 500) {
      ...
      for (final raw in engine.drainConsole()) {
        ...
        final message = separator < 0
            ? HtmlMlConsoleMessage('log', raw)
            : HtmlMlConsoleMessage(
  ...
  void _serviceHostRequests(HtmlMlEngine engine) {
        ...
        if (handler != null) {
          unawaited(
            Future<void>.sync(() => handler(_controller, decoded)).catchError((
              ...
              debugPrint('[HtmlML host request] $error\n$stack');
              widget.onError?.call(error);
            }),
          );
        }
  ...
  void _onPointerDown(PointerDownEvent event) {
    ...
    _pointer(htmlMlPointerDown, event, changedButton: event.buttons);
  ...
  void _onPointerUp(PointerUpEvent event) {
    ...
    _buttons = event.buttons;
    _pointer(htmlMlPointerUp, event, changedButton: released);
  }
  ...
  KeyEventResult _onKeyEvent(FocusNode node, KeyEvent event) {
    ...
    engine.enqueue(
      kind: down ? htmlMlKeyDown : htmlMlKeyUp,
    ...
    if (down &&
      ...
      for (final scalar in event.character!.runes) {
        engine.enqueue(kind: htmlMlText, x: scalar.toDouble());
      }
    }
  ...
  Widget build(BuildContext context) => Focus(
        ...
        child: MouseRegion(
          ...
          child: Listener(
            ...
            onPointerHover: (event) => _pointer(htmlMlPointerMove, event),
            onPointerMove: (event) {
              _buttons = event.buttons;
              _pointer(htmlMlPointerMove, event);
            },
            ...
            child: RepaintBoundary(
              child: CustomPaint(
                painter: _HtmlMlPainter(_projector),
...
final class _HtmlMlPainter extends CustomPainter {
  const _HtmlMlPainter(this.projector);

  final HtmlMlSceneProjector projector;

  @override
  void paint(ui.Canvas canvas, ui.Size size) => projector.paint(canvas, size);

  @override
  bool shouldRepaint(covariant _HtmlMlPainter oldDelegate) => true;
}
