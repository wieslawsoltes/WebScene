...
using create_fn = htmlml_engine* (*)(const htmlml_engine_options*);
using destroy_fn = void (*)(htmlml_engine*);
...
std::unordered_map<htmlml_engine*, std::unique_ptr<ResourceContext>> engines;
RuntimeApi runtime;
...
bool ensure_runtime(const char* path)
{
    ...
    if (path == nullptr || path[0] == '\0') {
        last_error = "The HtmlML native library path is empty.";
        return false;
    }
    ...
    RuntimeApi loaded{
        module,
        resolve<get_abi_version_fn>(module, "htmlml_engine_get_abi_version"),
        resolve<prewarm_fn>(module, "htmlml_engine_prewarm"),
        resolve<create_fn>(module, "htmlml_engine_create_with_options"),
        resolve<destroy_fn>(module, "htmlml_engine_destroy"),
    };
    if (loaded.get_abi_version == nullptr || loaded.prewarm == nullptr
        || loaded.create == nullptr || loaded.destroy == nullptr) {
        last_error = "The selected library does not export the HtmlML engine ABI.";
        dlclose(module);
        return false;
    }
    if (loaded.get_abi_version() != 2) {
        last_error = "The selected HtmlML library does not implement ABI version 2.";
        dlclose(module);
        return false;
    }
    runtime = loaded;
    ...
    int64_t modified)
{
    ...
    [request setValue:@"HtmlML-Flutter/0.1" forHTTPHeaderField:@"User-Agent"];
    if (!entity_tag.empty()) {
        NSString* value =
    ...
    htmlml_text_metrics* metrics)
{
    @autoreleasepool {
        if (text == nullptr || metrics == nullptr
            || metrics->struct_size < sizeof(htmlml_text_metrics) || font_size <= 0) {
            return 0;
        }
...
htmlml_engine* htmlml_flutter_engine_create(
    ...
    const char* cache_directory)
{
    @autoreleasepool {
        ...
        if (runtime.prewarm() == 0) {
            last_error = "HtmlML V8 prewarm failed.";
            return nullptr;
        }
        ...
        if (engine == nullptr) {
            last_error = "HtmlML engine creation failed.";
            return nullptr;
        }
...
extern "C" __attribute__((visibility("default")))
void htmlml_flutter_engine_destroy(htmlml_engine* engine)
{
...
extern "C" __attribute__((visibility("default")))
const char* htmlml_flutter_last_error(void)
{
    return last_error.c_str();
}

extern "C" __attribute__((visibility("default")))
uint64_t htmlml_flutter_resource_request_count(htmlml_engine* engine)
{
    std::lock_guard lock(engines_mutex);
    auto found = engines.find(engine);
