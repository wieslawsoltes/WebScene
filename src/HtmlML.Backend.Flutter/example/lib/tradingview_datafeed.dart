...
final class TradingViewExampleDatafeed {
  ...
  List<HtmlMlScript> get initializationScripts => [
        const HtmlMlScript('''
(() => {
...
''', documentName: 'htmlml-flutter-change-theme.js'),
        HtmlMlScript('''
(() => {
  if (typeof globalThis.__htmlMlPrimeTradingView === "function"
      && (!globalThis.TradingView || typeof globalThis.TradingView.widget !== "function")) {
    globalThis.__htmlMlPrimeTradingView();
  }
...
''', documentName: 'htmlml-flutter-set-symbol.js'),
      ];
    ...
    switch (request['kind']) {
      ...
      default:
        controller.executeScript(
          ...
          documentName: 'htmlml-flutter-unknown-host-request.js',
        );
    }
  }

  void _getBars(HtmlMlController controller, Map<String, dynamic> request) {
    try {
...
''', documentName: 'htmlml-flutter-history-response.js');
    } catch (error) {
      controller.executeScript('''
(() => {
...
''', documentName: 'htmlml-flutter-history-error.js');
    }
  }
