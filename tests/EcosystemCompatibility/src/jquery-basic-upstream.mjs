...
const selectedModules = new Set([
  "attributes", "css", "data", "dimensions", "event", "manipulation",
  "offset", "serialize", "wrap"
]);
globalThis.jQuery = globalThis.$ = globalThis.supportjQuery = jQuery;
globalThis.includesModule = name => selectedModules.has(name);
globalThis.moduleTeardown = () => {
  fixture.innerHTML = fixtureMarkup;
};

import("../upstream/jquery/test/unit/basic.js")
  .then(() => runRegisteredQUnitTests())
  .catch(error => {
    const state = globalThis.__webSceneWptState;
    state.results.push({
      name: "jQuery basic upstream source load",
      status: "FAIL",
