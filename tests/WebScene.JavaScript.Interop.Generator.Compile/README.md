...
`webscene-interop-discover`. It is intentionally not tied to TradingView.

```bash
dotnet build WebScene.JavaScript.Interop.Generator.Compile.csproj \
  -p:WebSceneInteropApiManifest=/absolute/path/library.webscene-interop-api.json \
  -p:WebSceneInteropPolicy=/absolute/path/library.webscene-interop-policy.json
```
...
assembly stale. Pass `-p:WebSceneInteropHarnessId=unique-name` when two inputs
