...
public sealed class GeneratedHandleReturnTests
{
    ...
    public async Task GeneratedIndexedModelsPreserveTypedKeysAndRetainedValues()
    {
        ...
        Assert.Contains(
            """void:acceptWidgetDictionary:{"sent":{"__htmlMlHandle":10}}""",
            invoker.Calls);
        Assert.Contains(
            """void:acceptGenericWidgetDictionary:{"genericSent":{"__htmlMlHandle":10}}""",
            invoker.Calls);
    ...
    public async Task GeneratedBindingsEncodeAndMaterializeTypeScriptTuples()
    {
        ...
        Assert.Contains(
            """void:acceptWidgetTuple:["sent",{"__htmlMlHandle":10}]""",
            invoker.Calls);
    ...
    public async Task GeneratedModelsMaterializeNestedRetainedValues()
    {
        var invoker = new NativeJavaScriptInvoker(
            (source, document, _) => Task.FromResult(document switch
            {
                ...
                "htmlml-interop-value.js" when source.Contains(
                    "\"snapshot\"",
                    StringComparison.Ordinal) =>
                    """
                    {
                      "dispose":{"__htmlMlHandle":601},
                      "maybeWidget":{"__htmlMlHandle":602},
                      "title":"state",
                      "tuple":["paired",{"__htmlMlHandle":603}],
                      "widget":{"__htmlMlHandle":604},
                      "widgets":[{"__htmlMlHandle":605}]
                    }
                    """,
                "htmlml-interop-value.js" when source.Contains(
                    "\"anonymousSnapshot\"",
                    ...
                    {
                      "title":"anonymous",
                      "widget":{"__htmlMlHandle":606}
                    }
                    """,
                "htmlml-interop-value.js" =>
                    """
                    {
                      "value":{"__htmlMlHandle":607},
                      "values":[
                        {"__htmlMlHandle":608},
                        {"__htmlMlHandle":609}
                      ]
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (method == "widgetDictionary")
            {
                return Result<T>(JsonSerializer.Deserialize(
                    """{"primary":{"__htmlMlHandle":217}}""",
                    typeof(T))!);
            }
            if (method == "genericWidgetDictionary")
            {
                return Result<T>(JsonSerializer.Deserialize(
                    """{"generic":{"__htmlMlHandle":221}}""",
                    typeof(T))!);
            }
            if (method == "numericWidgetDictionary")
            {
                return Result<T>(JsonSerializer.Deserialize(
                    """{"7":{"__htmlMlHandle":218}}""",
                    typeof(T))!);
            }
            if (method == "mixedWidgetDictionary")
            {
                return Result<T>(JsonSerializer.Deserialize(
                    """
                    {
                      "primary":{"__htmlMlHandle":219},
                      "secondary":{"__htmlMlHandle":220}
                    }
                    """,
                    typeof(T))!);
            }
