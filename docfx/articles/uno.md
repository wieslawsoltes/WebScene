...
> [!NOTE]
> Uno support is scoped to Skia desktop on the published native RIDs. Uno browser,
> Android, iOS, and non-Skia targets are not supported by the current native runtime
> packages. WebScene remains a bounded runtime for trusted application content, not a
> browser or sandbox.
...
The same API manifest, policy, and generated facade can live in a framework-neutral
class library shared by Avalonia and Uno hosts. See
