...
Avalonia's `NativeWebSceneView` and Uno's `UnoNativeWebSceneView` expose the same native
ABI 3 JavaScript boundary. The host view owns the V8 isolate; object references are
isolate-local handles; and generated calls use tagged request and result codecs rather
than building JavaScript source or serializing every call through JSON.
...
| Call an application or library API repeatedly | Generated bindings from `WebScene.JavaScript.Interop.Generator` |
| Receive JavaScript callbacks in .NET | Generated binary callback adapters |

## Evaluate small diagnostic expressions

Call evaluation only after `LoadAsync` has completed:

```csharp
string json = await webSceneView.EvaluateTextAsync(
...
## Use a generated facade from either host
...
using NativeJavaScriptInvoker invoker = webSceneView.CreateJavaScriptInvoker();
...
`AppEditor` and `AppEditorOptions` in this example represent generated types selected by
the application's policy. The generated class library is independent of Avalonia and
Uno; only the view used to create `NativeJavaScriptInvoker` changes.
...
install it with `NativeWebSceneLoadOptions.DocumentStartScripts` rather than evaluating
it after navigation:
