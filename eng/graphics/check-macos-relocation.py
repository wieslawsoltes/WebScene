...
def check(args):
    ...
    with tempfile.TemporaryDirectory(prefix="webscene-relocation-") as temporary:
        ...
        def run(command, directory, expected_libraries):
            ...
            for library in ["libEGL.dylib", "libGLESv2.dylib", "libwebgpu_dawn.dylib"]:
        ...
        shutil.copy2(args.sdk / "dawn/lib/libwebgpu_dawn.dylib", destination / "libwebgpu_dawn.dylib")
        for name, arguments in [("webscene_dawn_probe", ["metal"]),
            ...
            run([str(destination / name), *arguments], destination, libraries if "angle" in name else ["libwebgpu_dawn.dylib"])
        libraries.append("libwebgpu_dawn.dylib")
        for mode, source in [("enabled", args.native_enabled), ("disabled", args.native_disabled)]:
            ...
            if mode == "enabled":
                for name in libraries:
                    component = "dawn" if name == "libwebgpu_dawn.dylib" else "angle"
                    shutil.copy2(args.sdk / component / "lib" / name, directory / name)
