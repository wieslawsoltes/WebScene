...
`.github/workflows/graphics-sdk-build.yml` builds the pinned Dawn/ANGLE SDKs and links their diagnostic probes on hosted Windows and Linux runners. It deliberately does not run GPU probes or count successful compilation as hardware qualification. Verified installed SDKs are cached with `actions/cache/restore` and `actions/cache/save`, separately for each component and RID. Exact keys include the hosted runner image version and hashes of the dependency lock, build recipe, symbol/export policy, Windows SDK installer and workflow. Test-only and application changes reuse the same SDK. No prefix fallback is allowed; changed build inputs cause a rebuild. Every hit still passes full SDK inventory/settings/checksum verification and fresh consumer compile/link checks. Only verified SDKs are saved, immediately before consumer probes, so later probe failures do not discard successful dependency builds. The source and intermediate build trees are not cached.

The first run for each key must build it. Caches can also miss after eviction or runner image updates. A push to `main` seeds default-branch caches for subsequent PRs. Dispatch with `clean=true` to bypass restoration for a deliberate source rebuild. Bump the cache generation (`v1`) if an existing immutable entry needs replacing. Four component/platform jobs may run concurrently; actual runner availability still controls start times. This cache covers the hosted graphics SDK workflow; native runtime packaging retains its separate V8 cache.

Its SDK artifacts are retained for 14 days for inspection. The separate dedicated-runner workflow below owns hardware execution.
...
Current local macOS evidence covers both native backend probes, graphics-enabled/disabled V8-free parser builds, native/probe relocation, SDK mismatch rejection and 32 repeated hardware-Chrome courtyard/fixture/seeded 10k/100k reference runs. Remaining #23 gates include Windows/Linux builds and hardware runs, native V8 runtime/package integration, independently qualified GL where needed, complete durable reference artifact storage, and the complete non-GPU baseline. Unchanged Kestrel now passes scoped macOS WebScene WebGPU startup, pan, sidebar and resize probes; see `docs/graphics/coherent-gpu-scene-publication.md` for exact evidence and limitations. These application checks do not establish complete WebGPU/WebGL API support, full conformance or physical 60fps qualification. Keep #23 open until all its acceptance evidence is complete. At the user's direction, #24 implementation proceeds while #23 CI and hardware qualification remain outstanding; this does not waive either issue's acceptance gates.
...
The pinned ANGLE toolchain requires Windows SDK headers/libraries `10.0.28000.0`, including x86 tools used during GN configuration. Hosted ANGLE CI explicitly runs `eng/graphics/install-windows-sdk.ps1` before initializing Visual C++. This pins the Microsoft `10.0.28000.2526` installer, verifies its SHA-256 and Authenticode signature, installs without restarting, and checks the required x86/x64 headers, libraries and resource compiler. Developers and dedicated runner administrators may explicitly run that script from PowerShell 7 with installation privileges; `build.py` does not install system software automatically.

Installer provenance: [Microsoft SDK downloads](https://learn.microsoft.com/en-us/windows/apps/windows-sdk/downloads) and [Microsoft WinGet manifest](https://github.com/microsoft/winget-pkgs/blob/master/manifests/m/Microsoft/WindowsSDK/10/0/28000/10.0.28000.2526/Microsoft.WindowsSDK.10.0.28000.installer.yaml). Downloaded installer bytes matched the published SHA-256 locally. Windows installation and subsequent ANGLE compilation still require CI verification.
